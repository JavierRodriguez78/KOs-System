...
namespace kos { 
    namespace kernel {
        ...
        void InitDrivers(InterruptManager* interrupts)
        {
            ...
            // Avoid intrusive PS/2 reprogramming after interrupts are live.
            // VirtualBox can stall here, which prevents the kernel from reaching
            // graphics initialization and hides the VMSVGA diagnostics we need.
            g_kbd_poll_enabled = false;
            Logger::LogStatus("Keyboard verification skipped", true);
        }
    }
 }
