...
namespace kos { namespace console { namespace nc {

// Minimal ncurses-like API for kernel terminal
// Functions are intentionally simple and synchronous.

inline void init() {
    // No special init required; ensure terminal buffer is clean
    TTY::Clear();
}
...
inline void set_color(uint8_t fg, uint8_t bg) {
    TTY::SetColor(fg, bg);
}

inline void move(uint32_t col, uint32_t row) {
    // Move cursor via TTY API
    TTY::MoveCursor(col, row);
}

inline void addch(char c) {
    TTY::PutChar((int8_t)c);
}

inline void addstr(const char* s) {
    TTY::Write((const int8_t*)s);
}
