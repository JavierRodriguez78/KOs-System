...
enum class LoginAction : uint8_t {
    None = 0,
    Reboot,
    Shutdown
};
...
public:
    ...
    // Handle mouse click in absolute screen coordinates
    static void OnPointerDown(int x, int y);
    // Returns true once authentication succeeded
    static bool Authenticated();
    // Get entered username (for post-login greeting)
    static const char* Username();
    // Consume one pending action requested by login UI.
    static LoginAction ConsumePendingAction();
private:
    static uint32_t s_win_id;
    static bool s_ready;
    static bool s_authenticated;
    static const int kMaxListedUsers = 16;
    static char s_users[kMaxListedUsers][32];
    static int  s_user_count;
    static int  s_selected_user;
    ...
    static uint8_t s_session_idx; // selected desktop session
    static char s_message[96]; // status / error text
    static LoginAction s_pending_action;
    static bool s_user_menu_open;
    static void drawText(uint32_t x, uint32_t y, const char* text, uint32_t fg, uint32_t bg);
    static void loadUsersFromService();
    static void selectUser(int idx);
